...
gulp.task("copy", function () {
  // コピー
  var subTasks = [
    gulp.src(`${SOURCE_DIR}/mimetype`).pipe(gulp.dest(`${BUILD_DIR}`)),

    gulp.src(`${SOURCE_META_DIR}/**/*`).pipe(gulp.dest(BUILD_META_DIR)),

    gulp.src(`${SOURCE_IMAGE_DIR}/**/*.png`).pipe(gulp.dest(BUILD_IMAGE_DIR)),

    gulp.src(`${SOURCE_IMAGE_DIR}/**/*.jpg`).pipe(gulp.dest(BUILD_IMAGE_DIR)),

    gulp.src(`${SOURCE_IMAGE_DIR}/**/*.svg`).pipe(gulp.dest(BUILD_IMAGE_DIR)),
  ];
...
gulp.task("css", async function () {
  // CSS生成
  await gulp
    .src(`${SOURCE_STYLES_DIR}/epub.scss`)
...
gulp.task("css-print", async function () {
  // CSS生成
  await gulp
    .src(`${SOURCE_STYLES_DIR}/print.scss`)
    .pipe(sass({ outputStyle: "expanded" }).on("error", sass.logError))
    .pipe(concat("style.css"))
    .pipe(gulp.dest(BUILD_STYLES_DIR));
});
...
gulp.task("css-pod", async function () {
  // CSS生成
  await gulp
    .src(`${SOURCE_STYLES_DIR}/pod.scss`)
    .pipe(sass({ outputStyle: "expanded" }).on("error", sass.logError))
    .pipe(concat("style.css"))
    .pipe(gulp.dest(BUILD_STYLES_DIR));
});
...
async function resizeImages(contents) {
  ...
  for (let x of images) {
    ...
    // 塗り足し部分をクロップする
    let cropConfig = bookConfig.epubImageCrops.find((y) =>
      new RegExp(y.fileNamePattern).test(x.fileName),
    );
    ...
    let isKindleOk =
      (isPortrait && width >= 1200 && height >= 1920) ||
...
async function generateHtmlByImage(targetImage, config) {
  ...
  frontmatter.viewport = frontmatter.viewport
    ? frontmatter.viewport
    : `width=${targetImage.dimensions.width}, height=${targetImage.dimensions.height}`;
...
async function build(buildType, enableImageResizing) {
  ...
  // イメージファイルのスキャン
  let contents = (await fsp.readdir(BUILD_IMAGE_DIR)).map((x) => ({
    type: "image",
    fileName: x,
  ...
  contents = contents.filter((x) => x.id != "cover");
  ...
  // 除外コンテンツを削除
  let filesToRemove = (await fsp.readdir(BUILD_XHTML_DIR)).filter(
    (x) => !contents.some((y) => x === y.fileName),
  );
