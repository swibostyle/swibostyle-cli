...
async function runPrompts(
  ...
  // Template
  const template = cliOptions.template || (await p.select({
  ...
  // Language
  const lang = cliOptions.lang || (await p.text({
  ...
  // Page direction
  const pageDirection = cliOptions.direction || (await p.select({
  ...
  // Writing mode
  const writingMode = cliOptions.writingMode || (await p.select({
  ...
  // Package manager
  const packageManager = cliOptions.pm || (await p.select({
